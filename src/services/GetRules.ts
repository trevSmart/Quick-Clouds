...
function getRules(storageManager, context, instanceIdOverride) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        let url = extension_1.env + '/api/v2/rule?filter[instance_id]=';
        let instanceId;
        if (instanceIdOverride) {
            instanceId = instanceIdOverride;
        }
        else {
            let user = yield storageManager.getUserData('userInfo');
            if (!user) {
        ...
        try {
            ...
            if (axios_1.default.isAxiosError(error)) {
                ...
                if (authType === 'credentials') {
                    ...
                    console.error('Error in getRules:', (_a = error.response) === null || _a === void 0 ? void 0 : _a.statusText);
                }
            }
            else {
